...
func IndexData(title, pageText, link string) {
	// index data to elasticsearch
}

func extractContent(link *string, crawledLinksChannel chan string) {
	...
	if title != "" && pageText != "" {
		fmt.Println(*link)
		IndexData(title, pageText, *link)
	}

}

func CrawlWebpage(wg *sync.WaitGroup, pendingLinksChannel chan string,
	crawledLinksChannel chan string, linksAmountChannel chan int, depth int) {

	for link := range pendingLinksChannel {
		extractContent(&link, crawledLinksChannel)
		linksAmountChannel <- -1
		depth--
		if depth == 0 {
			break
...
func CrawlerMain(startLinks []string, depth int, numThreads int) {
	...
	for i := 0; i < numThreads; i++ {
		wg.Add(1)
		go CrawlWebpage(&wg, pendingLinksChannel, crawledLinksChannel, linksAmountChannel, depth)
	}
